...
if __name__ == "__main__":
    ...
    if 'resume_analyzed' not in st.session_state:
        st.session_state['resume_analyzed'] = False # State flag to control visibility
    ...
    uploaded_file = st.file_uploader(
        "Option 1: Drag and drop your resume file here (.txt or .pdf):",
        type=['txt', 'pdf'], # <--ACCEPTS PDF
        ...
        try:
            if file_extension == '.pdf':
                ...
                for page in pdf_reader.pages:
                    # Note: We use 'or ""' to handle pages that might have no extractable text
    ...
    # Use a key to ensure this button's state is distinct
    if st.button("Analyze Resume", key="analyze_button"):
        ...
        # Extract keywords (kept your original keyword extraction logic)
   ...
   # Check session state to display subsequent widgets on all returns
    if st.session_state['resume_analyzed'] and st.session_state['job_options']:
        ...
        # Check if roadmap has been generated and stored
